...
logger = logging.getLogger("fusionaize_docs")
...
def _anthropic_bridge_response_headers(
    *,
    ...
    headers = {
        "X-fusionaize_docs-Bridge-Surface": "anthropic-messages",
        "X-fusionaize_docs-Bridge-Source": _sanitize_token(source, default="claude-code", max_chars=64),
        "X-fusionaize_docs-Bridge-Model-Requested": _sanitize_token(
            requested_model,
            default="unknown",
            max_chars=96,
        ),
    ...
    if resolved_model and resolved_model != requested_model:
        headers["X-fusionaize_docs-Bridge-Model-Resolved"] = _sanitize_token(
            resolved_model,
            default="unknown",
            max_chars=96,
        )
    if anthropic_version:
        headers["X-fusionaize_docs-Bridge-Anthropic-Version"] = _sanitize_token(
            anthropic_version,
            default="unknown",
            max_chars=64,
        )
    if anthropic_beta:
        headers["X-fusionaize_docs-Bridge-Anthropic-Beta"] = _sanitize_token(
            anthropic_beta,
            default="unknown",
            max_chars=96,
        )
...
def _collect_routing_headers(request: Request) -> dict[str, str]:
    ...
    prefixes = ("x-openclaw", "x-fusionaize_docs")
...
def _collect_anthropic_bridge_headers(request: Request) -> dict[str, str]:
    ...
    bridge_source = _sanitize_token(
        ...
        or request.headers.get("x-fusionaize_docs-client")
        or request.headers.get("x-claude-code-client")
        or "claude-code",
        default="claude-code",
        max_chars=max_chars,
    )
    headers.setdefault("x-fusionaize_docs-client", bridge_source)
    headers.setdefault("x-fusionaize_docs-surface", "anthropic-messages")
...
def _collect_operator_context(headers: dict[str, str]) -> tuple[str, str]:
    ...
    action = _sanitize_token(
        headers.get("x-fusionaize_docs-operator-action", "update-check"),
        default="update-check",
        max_chars=max_chars,
    )
    client_tag = _sanitize_token(
        headers.get("x-fusionaize_docs-client", "operator"),
        default="operator",
        max_chars=max_chars,
    )
...
def _resolve_client_tag(headers: dict[str, str], client_profile: str) -> str:
    ...
    if headers.get("x-fusionaize_docs-client"):
        return _sanitize_token(
            headers["x-fusionaize_docs-client"],
...
def _merge_routing_context_headers(headers: dict[str, str], body: dict[str, Any]) -> dict[str, str]:
    ...
    if cache_preference and "x-fusionaize_docs-cache" not in merged:
        merged["x-fusionaize_docs-cache"] = cache_preference
    image_policy = _collect_request_image_policy(body)
    if image_policy and "x-fusionaize_docs-image-policy" not in merged:
        merged["x-fusionaize_docs-image-policy"] = image_policy
    return merged
    ...
    for provider_name in attempt_order:
        ...
        try:
            ...
            _adaptive_state.record_success(
                provider_name,
                latency_ms=(result.get("_fusionaize_docs") or {}).get("latency_ms", 0) if isinstance(result, dict) else 0.0,
            )

            trace_id: str | None = None
            if _config.metrics.get("enabled") and isinstance(result, dict):
                usage = result.get("usage", {})
                cg = result.get("_fusionaize_docs", {})
...
def _openai_result_to_canonical_response(result: dict[str, Any]) -> CanonicalChatResponse:
    ...
    provider_meta = result.get("_fusionaize_docs") or {}
...
async def lifespan(app: FastAPI):
    ...
    _update_checker = UpdateChecker(
        ...
        repository=str(_config.update_check.get("repository", "fusionAIze/fusionaize_docs")),
    ...
    try:
        ...
        if source_refresh_cfg.get("enabled") and int(source_refresh_cfg.get("interval_seconds") or 0) > 0:
            _provider_catalog_refresh_task = asyncio.create_task(
                ...
                name="fusionaize_docs-provider-source-refresh",
            )
    except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            if _config.metrics.get("enabled") and isinstance(result, dict):
                row_id = _metrics.log_request(
                    ...
                    layer=decision.layer,
                    rule_name=decision.rule_name,
                    latency_ms=(result.get("_fusionaize_docs") or {}).get("latency_ms", 0),
                    requested_model=model_requested,
            ...
            resp.headers["X-fusionaize_docs-Provider"] = provider_name
            resp.headers["X-fusionaize_docs-Profile"] = client_profile
            if resolved_mode:
                resp.headers["X-fusionaize_docs-Mode"] = resolved_mode
            if resolved_shortcut:
                resp.headers["X-fusionaize_docs-Shortcut"] = resolved_shortcut
            resp.headers["X-fusionaize_docs-Layer"] = decision.layer
            resp.headers["X-fusionaize_docs-Rule"] = decision.rule_name
            resp.headers["X-fusionaize_docs-Hooks"] = ",".join(hook_state.applied_hooks)
            resp.headers["X-fusionaize_docs-Hook-Errors"] = str(len(hook_state.errors))
            resp.headers["x-fusionaize_docs-trace-id"] = trace_id or str(uuid.uuid4())
            return resp
        except ProviderError as exc:
...
async def chat_completions(request: Request):
    ...
    if execution.stream:
        return StreamingResponse(
            ...
            headers={
                "X-fusionaize_docs-Provider": execution.provider_name,
                "X-fusionaize_docs-Profile": execution.client_profile,
                "X-fusionaize_docs-Hooks": ",".join(execution.hook_state.applied_hooks),
                "X-fusionaize_docs-Hook-Errors": str(len(execution.hook_state.errors)),
                "x-fusionaize_docs-trace-id": execution.trace_id or str(uuid.uuid4()),
            },
        )

    resp = JSONResponse(execution.result)
    resp.headers["X-fusionaize_docs-Provider"] = execution.provider_name
    resp.headers["X-fusionaize_docs-Profile"] = execution.client_profile
    if execution.resolved_mode:
        resp.headers["X-fusionaize_docs-Mode"] = execution.resolved_mode
    if execution.resolved_shortcut:
        resp.headers["X-fusionaize_docs-Shortcut"] = execution.resolved_shortcut
    resp.headers["X-fusionaize_docs-Layer"] = execution.decision.layer
    resp.headers["X-fusionaize_docs-Rule"] = execution.decision.rule_name
    resp.headers["X-fusionaize_docs-Hooks"] = ",".join(execution.hook_state.applied_hooks)
    resp.headers["X-fusionaize_docs-Hook-Errors"] = str(len(execution.hook_state.errors))
    resp.headers["x-fusionaize_docs-trace-id"] = execution.trace_id or str(uuid.uuid4())
    return resp
...
async def anthropic_messages(request: Request):
    ...
    if execution.stream:
        return StreamingResponse(
            ...
            headers={
                "X-fusionaize_docs-Provider": execution.provider_name,
                "X-fusionaize_docs-Profile": execution.client_profile,
                "X-fusionaize_docs-Layer": execution.decision.layer,
                "X-fusionaize_docs-Rule": execution.decision.rule_name,
                "X-fusionaize_docs-Hooks": ",".join(execution.hook_state.applied_hooks),
                "X-fusionaize_docs-Hook-Errors": str(len(execution.hook_state.errors)),
                "x-fusionaize_docs-trace-id": execution.trace_id or str(uuid.uuid4()),
                **bridge_headers,
    ...
    response.headers["X-fusionaize_docs-Provider"] = execution.provider_name
    response.headers["X-fusionaize_docs-Profile"] = execution.client_profile
    response.headers["X-fusionaize_docs-Layer"] = execution.decision.layer
    response.headers["X-fusionaize_docs-Rule"] = execution.decision.rule_name
    response.headers["X-fusionaize_docs-Hooks"] = ",".join(execution.hook_state.applied_hooks)
    response.headers["X-fusionaize_docs-Hook-Errors"] = str(len(execution.hook_state.errors))
    response.headers["x-fusionaize_docs-trace-id"] = execution.trace_id or str(uuid.uuid4())
...
async def anthropic_count_tokens(request: Request):
    ...
    bridge_headers = _anthropic_bridge_response_headers(
        source=str(headers.get("x-fusionaize_docs-client") or "claude-code"),
...
def main():
    """Run with: python -m fusionaize_docs"""
    import uvicorn

    parser = argparse.ArgumentParser(
        prog="fusionaize_docs",
