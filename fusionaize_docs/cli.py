#!/usr/bin/env python3
"""fusionaize_docs-stats – CLI dashboard for fusionAIze Gate metrics.

Usage:
    python -m fusionaize_docs.cli              # Full overview
    python -m fusionaize_docs.cli --recent 20  # Last 20 requests
    python -m fusionaize_docs.cli --daily      # Daily cost breakdown
    python -m fusionaize_docs.cli --json       # JSON output (pipe-friendly)
"""
...
def cmd_overview(metrics: MetricsStore, **filters):
    ...
    print(f"  {_c('Generate deep link:', DIM)} {_c(f'fusionaize_docs-stats --link --view overview {filter_str}', DIM)}")
    print()
...
def cmd_recent(metrics: MetricsStore, limit: int, **filters):
    ...
    for key, value in filters.items():
        if key == "success":
            ...
            filter_args.append(f"--{key.replace('_', '-')} {value}")
    filter_str = " ".join(filter_args)
    print(f"  {_c('See more recent:', DIM)} {_c(f'fusionaize_docs-stats --link --view overview {filter_str}', DIM)}")
    print()
...
def cmd_daily(metrics: MetricsStore, days: int, **filters):
    ...
    print(f"  {_c('Generate deep link:', DIM)} {_c('fusionaize_docs-stats --link --view analytics', DIM)}")
    print()
...
def cmd_suggest(metrics: MetricsStore, **filters):
        ...
        suggestions.append(
            {
                ...
                "command": "fusionaize_docs-stats --recent 20 --success false",
                ...
                suggestions.append(
                    {
                        ...
                        "command": f"fusionaize_docs-stats --provider {provider_name}",
        ...
        suggestions.append(
            {
                ...
                "command": "fusionaize_docs-stats --daily --days 30",
            ...
            suggestions.append(
                {
                    ...
                    "command": "fusionaize_docs-stats --recent 20",
    ...
    suggestions.append(
        {
            ...
            "command": "fusionaize_docs-stats --link",
    ...
    if not suggestions:
        ...
        print(_c("  Try:", DIM))
        print(_c("    • fusionaize_docs-stats --overview", DIM))
        print(_c("    • fusionaize_docs-stats --link", DIM))
        print()
        return
